...
def install_process_test_services(tmp_path: Path, responses: list[list[DetectedFace]]) -> tuple[Any, Any, Any, Any, Any, Any]:
    ...
    original_archive_service = process_api.archive_service
    ...
    process_api.archive_service = process_api.CropArchiveService(output_base_dir=tmp_path / "runs")

    return process_api, config_module, original_service, original_staging_service, original_archive_service, original_settings


def restore_process_test_services(
    process_api: Any,
    config_module: Any,
    original_service: Any,
    original_staging_service: Any,
    original_archive_service: Any,
    original_settings: Any,
) -> None:
    process_api.service = original_service
    process_api.staging_service = original_staging_service
    process_api.archive_service = original_archive_service
...
def test_process_prefers_best_match_across_rotations(tmp_path: Path) -> None:
    ...
    try:
        ...
        run_id = process_response.json()["runId"]
        download_response = client.get(f"/process/{run_id}/download")
    finally:
        restore_process_test_services(process_api, config_module, original_service, original_staging_service, original_archive_service, original_settings)
    ...
    assert any(member.startswith(f"face-search-results-{run_id}/") for member in members)
    assert any(member.endswith('.jpg') for member in members)
